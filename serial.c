 ...
 * val = value
 */
void exec_cmd(char *name, float val)
{
	...
	if ((pid = fork()) == 0) {
		...
		if (execlp("rrdtool", "touch", "update", name, cmd, NULL) == -1)
			exit(-1);
	}
	if (pid < 0)
		return;
...
void kw1281_handle_error()
{
	 ...
	 */
	printf("TRYING TO RECOVER WITH SENDING REQUEST\n");
